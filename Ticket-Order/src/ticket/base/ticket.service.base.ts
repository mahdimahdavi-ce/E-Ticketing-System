...
import { Prisma, Ticket, Travel, UserTicket } from "@prisma/client";
...
export class TicketServiceBase {
  ...
  async getTravelId(parentId: string): Promise<Travel | null> {
  ...
  async getUserId(parentId: string): Promise<UserTicket | null> {
    return this.prisma.ticket
      .findUnique({
        where: { id: parentId },
      })
